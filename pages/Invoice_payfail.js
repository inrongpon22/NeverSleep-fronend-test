...
function Invoice_payfail() {
  //Get JSON
  const getUser = getData.data.users;
  const payFail = [];
    // Filter WaitpaymentusersData
    function allWaitStatus() {
      const nameOfStatus = getUser.map(namestatus => namestatus.status['name_status'].indexOf('ไม่สำเร็จ'));
        for (let i = 0; i < nameOfStatus.length; i++) {
        if (nameOfStatus[i] === 0) {
          payFail.push(getUser[i])
        }
      }
    }
    allWaitStatus()
  ...
  // Users Data
  const [users, setUsers] = useState(payFail);
          ...
          <div className="mb-5 drop-shadow-xl">
            <div id="tab" className="h-11 flex justify-around items-end">
              ...
              <Link href='/Invoice_payfail'>
                ...
                >
                  <h3
                    className="statustab-h3-active bg-gray-400"
                  >
                    ไม่สำเร็จ({totalUsers})
                  </h3>
                </div>
              </Link>
              ...
              <div id="table" className="h-72">
                <table className="w-full">
                  ...
                  <tbody id="total" className="">
                    {currentUser.map((payfailusers) => (
                      <tr>
                        <td key={payfailusers.id} className="pl-12">
                          {payfailusers.id}
                        </td>
                        <td key={payfailusers.code} className="text-blue-400">
                          {payfailusers.code}
                        </td>
                        <td key={payfailusers.project_name} className="text-blue-500">
                          {payfailusers.project_name}
                        </td>
                        <td key={payfailusers.data} className="">
                          {payfailusers.data.slice(0, 10)}
                        </td>
                        <td key={payfailusers.first_name} className="">
                          {payfailusers.first_name}
                        </td>
                        <td key={payfailusers.payment} className="text-red-500 text-lg font-semibold">
                          {payfailusers.payment.toLocaleString("en-US")}
                        </td>
                        <td key={payfailusers.slip}>
                          <a href="#" className="">
                            <RiBillFill className='text-blue-500 text-2xl' />
                          </a>
                        </td>
                        <td key={payfailusers.status} className="text-center">
                          <h4 className='bg-gray-200 rounded-lg w-5/6'>
                            {payfailusers.status.name_status}
                            </h4>
                          <div className="flex">
                            <p>โดย</p>
                            <FaUser className='text-gray-600' />
                            <p>{payfailusers.status.id}</p>
                            <FaUser className='text-gray-600' />
                            <p>{payfailusers.status.id}</p>
                          </div>
                        </td>
...
export default Invoice_payfail