...
function Invoice_pending() {
  //Get JSON
  const getUser = getData.data.users;
  const pendingPay = [];
    // Filter WaitpaymentusersData
    function allWaitStatus() {
      const nameOfStatus = getUser.map(namestatus => namestatus.status['name_status'].indexOf('รอตรวจสอบ'));
        for (let i = 0; i < nameOfStatus.length; i++) {
        if (nameOfStatus[i] === 0) {
          pendingPay.push(getUser[i])
        }
      }
    }
    allWaitStatus()
  ...
  // Users Data
  const [users, setUsers] = useState(pendingPay);
          ...
          <div className="mb-5 drop-shadow-xl">
            <div id="tab" className="h-11 flex justify-around items-end">
              ...
              <Link href='/Invoice_pending'>
                ...
                >
                  <h3
                    className="statustab-h3-active"
                  >
                    รอตรวจสอบ({totalUsers})
                  </h3>
                </div>
              </Link>
              ...
              <div id="table" className="h-72">
                <table className="w-full">
                  ...
                  <tbody id="total" className="">
                    {currentUser.map((pendingusers) => (
                      <tr>
                        <td key={pendingusers.id} className="pl-12">
                          {pendingusers.id}
                        </td>
                        <td key={pendingusers.code} className="text-blue-400">
                          {pendingusers.code}
                        </td>
                        <td key={pendingusers.project_name} className="text-blue-500">
                          {pendingusers.project_name}
                        </td>
                        <td key={pendingusers.data} className="">
                          {pendingusers.data.slice(0, 10)}
                        </td>
                        <td key={pendingusers.first_name} className="">
                          {pendingusers.first_name}
                        </td>
                        <td key={pendingusers.payment} className="text-red-500 text-lg font-semibold">
                          {pendingusers.payment.toLocaleString("en-US")}
                        </td>
                        <td key={pendingusers.slip}>
                          <a href="#" className="">
                            <RiBillFill className='text-blue-500 text-2xl' />
                          </a>
                        </td>
                        <td key={pendingusers.status} className="text-center">
                          <h4 className='bg-blue-200 rounded-lg w-5/6'>
                            {pendingusers.status.name_status}
                            </h4>
                          <div className="flex">
                            <p>โดย</p>
                            <FaUser className='text-gray-600'/>
                            <p>{pendingusers.status.id}</p>
                            <FaUser className='text-gray-600' />
                            <p>{pendingusers.status.id}</p>
                          </div>
                        </td>
...
export default Invoice_pending